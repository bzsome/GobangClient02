...
public class ChessBroad extends JPanel {
    ...
    private static final ChessBroad chessBroad = new ChessBroad();
    ...
    static {
        chessBroad.setVisible(true);
        // 设置鼠标形状为手型
        chessBroad.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        chessBroad.setBackground(new Color(249, 214, 91));
        chessBroad.addListener();
    }

    private ChessBroad() {
    }

    public static ChessBroad getInstance() {
        return chessBroad;
    }
    ...
    public static void init() {
        chessSize = chessBroad.getWidth() / 19;
        paintTable();
        chessBroad.repaint();
    }

    public static void addGrade() {
        if (PlayerMe.getInstance().getColor() != TableData.getTheColor()) {
            //我方胜利
            UserAuthService userService = new UserAuthImpl();
            userService.addGrade(100);// 向服务器发送更新积分请求！
            int grade = PlayerMe.getInstance().addGrade(100);
    ...
    private void addListener() {
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (GameCenter.getMode() == GameCenter.MODE_ONLINE || GameCenter.getMode() == GameCenter.MODE_GOBANG_SEVER) {
                    if (PlayerMe.getInstance().getColor() == TableData.getTheColor()) {
    ...
    public static void submitPaint(Spot spot) {
        ...
        if (TableData.isOver()) {
            ...
            if (TableData.getTheColor() == Spot.whiteChess) {
                // 当前应该下白棋，则黑棋获胜
                JOptionPane.showMessageDialog(MainFrame.getInstance(), "黑棋获胜！增加100积分！",
                        "游戏结束", JOptionPane.CANCEL_OPTION);
            } else {
                JOptionPane.showMessageDialog(MainFrame.getInstance(), "白棋获胜！增加100积分！",
                        "游戏结束", JOptionPane.CANCEL_OPTION);
            }
        }
    }
    ...
    private static void paintChess(Spot spot) {
        if (spot != null) {
            ...
            Graphics g = chessBroad.getGraphics();
    ...
    private static void paintTable() {
        final Graphics g = chessBroad.getGraphics();
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (GameCenter.isEnd()) {
                    ...
                    Graphics2D g = (Graphics2D) chessBroad.getGraphics();
